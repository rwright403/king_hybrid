...
class pressurized_liq_fuel_tank(BaseTank):
    def __init__(self, timestep, m_fuel, m_pres, P_pres_tank, P_atm, T_atm, rho_atm, V_fuel_tank, V_pres_tank, diam_out, diam_in, rho_wall, k_w, Cd, A_inj, fuel_str, pres_str, fill_type:FillType ):
        ...
        self.m_pres = m_total 
        self.V_tank = V_pres_total #this is messy fix
        self.height_tank = self.V_tank/self.Tank_Inner_Area #resolve tank height for new volume
    ...
    def system_of_fuel_pres_odes(self, t, y, P_cc):               
        ...
        Q_dot_fuel_wall_to_fuel = solve_Q_dot_natural_convection_liq(self.rho_fuel, T_wall_fuel, T_fuel, T_fuel, P_tank, 0.021, 0.4, h_fuel_wall, (np.pi*self.diam_in*h_fuel_wall + self.Tank_Inner_Area), self.fuel_str ) #relative to fuel cv       
        # Heat transfer (5) [natural convection] from pres to pres wall
        Q_dot_pres_wall_to_pres = solve_Q_dot_natural_convection_gas(rho_pres, T_wall_pres, T_pres, T_pres, P_tank, 0.021, 0.4, h_pres_wall, (np.pi*self.diam_in*h_pres_wall + self.Tank_Inner_Area), self.pres_str ) #relative to pres cv
        ...
        U_dot_pres = -P_tank*(-1*V_dot_fuel) + Q_dot_pres #NOTE: V_dot_fuel = V_dot_pres


        T_dot_fuel = (U_dot_fuel - u_fuel*m_dot_fuel)/(m_fuel*cv_fuel)


        rho_dot_pres = -(self.m_pres/(V_pres**2))*(-1*V_dot_fuel) #NOTE: V_dot_fuel = V_dot_pres
    ...
    def tank_ode_system(self, t, y, P_cc):
        ...
        if m_fuel > 0:
            ...
            return [self.T_fuel, self.T_pres, self.m_fuel, self.T_wall_fuel, self.T_wall_pres, self.rho_dot_pres]

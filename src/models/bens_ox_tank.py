...
class model():
    ...
    def inst(self,P_cc):
        ...
        ###use chosen injector model:
        if(self.inj_model == 1):
            m_dot_spi = self.Cd_1*self.A_inj_1 * np.sqrt( 2 * self.rho_exit * (self.P_tank - self.P_cc)  )
            self.m_dot_ox = m_dot_spi

        elif(self.inj_model == 2):
            if self.x_tank < 1:
                h_vap_exit = CP.PropsSI('H', 'Q', 1, 'P', self.P_cc, 'N2O')
                m_dot_hem = self.Cd_1*self.A_inj_1 * rho_vap_exit * np.sqrt( 2 * (h_tank_exit -  h_vap_exit) )

...
def solve_m_dot_in_upstream_components(m_dot_in_upstream, h_in_upstream, P_tank):
    ...
    if x >= 1.0:
        x = 1.0               #x=1 means all the mass in is a vapor
    elif x < 0.0:
        x = 0.0                #x=0 means all the mass in is a liquid
...
def solve_m_dot_liq_gas(m_dot_evap, m_dot_cond, m_dot_inj, m_dot_vent, m_dot_liq_in, m_dot_gas_in):
    ...
    m_dot_gas =  (m_dot_vent + m_dot_gas_in) + m_dot_evap - m_dot_cond #only m_dot_vent, m_dot_gas_in sign convention rel to gas node, also signed (-) from inj: for remaining terms, convert sign convention from liq cv to gas cv
...
def build_tank_initial_state(P_tank, V_tank, m_ox, T_atm):
...
def update_tank_initial_input(u, P_downstream, m_dot_in_upstream, h_in_upstream): 
    u[0] = P_downstream
    u[1] = m_dot_in_upstream
    u[2] = h_in_upstream
...
def resolve_tank_outputs(out, t, y, timestep, u, p, guess):
    ...
    P_downstream, m_dot_in_upstream, h_in_upstream = u
...
def rhs(t, y, timestep, u, p, guess):
    ...
    #solve m_dot_upstream in and distribution from
    m_dot_liq_in, m_dot_gas_in = solve_m_dot_in_upstream_components(m_dot_in_upstream, h_in_upstream, P_tank)
    ...
    delta_T =(T_wall_liq - T_wall_gas)
    if m_dot_gas_wall < 0.0:
        delta_T = delta_T =(T_wall_gas - T_wall_liq)


    #NOTE: for T_dot_wall_liq:  IN: (6)      OUT: (4) AND (8)
    T_dot_wall_liq = ( Q_dot_atm_to_liq_wall - Q_dot_liq_wall_to_liq - Q_dot_liq_wall_to_gas_wall + m_dot_liq_wall*CW*delta_T ) / (CW*m_liq_wall)

    #NOTE: for T_dot_wall_vap:  IN: (7) and (8)      OUT: (5)
    T_dot_wall_gas = ( Q_dot_atm_to_gas_wall - Q_dot_gas_wall_to_gas + Q_dot_liq_wall_to_gas_wall + m_dot_gas_wall*CW*delta_T ) / (CW*m_gas_wall)

    """
    a =  Q_dot_atm_to_gas_wall/ (CW*m_gas_wall)
    b =  - Q_dot_gas_wall_to_gas/ (CW*m_gas_wall)
    c =  Q_dot_liq_wall_to_gas_wall/ (CW*m_gas_wall)
    d =  m_dot_gas_wall*CW*(T_wall_liq - T_wall_gas)/ (CW*m_gas_wall)

    print("T_dot_wall_gas: ", T_dot_wall_gas, a,b,c,d)
    """
...
def main():
    ...
    botl_y, botl_rho_liq, botl_rho_gas = build_tank_initial_state(botl_P_tank, botl_V_tank, botl_m_nos, T_atm)
    ...
    run_tank_y, run_tank_rho_liq, run_tank_rho_gas = build_tank_initial_state(run_tank_P_tank, run_tank_V_tank, run_tank_m_nos, T_atm) #NOTE: 0.4 kg seed mass
    ...
    try:
        ...
        while(pcnt_fill < full_pcnt_fill) and (t < max_sim_time): 
            ...
            if step_idx % log_every == 0:
                ...
                m_dot_liq_in, m_dot_gas_in = solve_m_dot_in_upstream_components(botl_out[6], botl_out[5], botl_out[0])

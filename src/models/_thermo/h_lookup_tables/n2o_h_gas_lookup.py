...
class N2OHGasTable:
    def __init__(self, path="src/models/_thermo/h_lookup_tables/n2o_h_gas_table.npz"):
        d = np.load(path)
        self.rho_grid = d["rho_grid"]
        self.T_grid = d["T_grid"]
        self.h_gas_tab = d["h_gas_tab"]
    ...
    def lookup(self, rho, T):
        ...
        if np.isnan(Q11) or np.isnan(Q21) or np.isnan(Q12) or np.isnan(Q22):
            print(f"Caution: h_gas lookup returned nan at (rho={rho:.0f} kg/m^3), (T={T:.4f} K), using lib directly (approx .012s per call)")
            return SpanWagnerEOS_SingleState(rho, T).h

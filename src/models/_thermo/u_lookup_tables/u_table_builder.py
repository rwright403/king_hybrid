...
gas_out_file = os.path.join(SCRIPT_DIR, "n2o_u_gas_table.npz")
liq_out_file = os.path.join(SCRIPT_DIR, "n2o_u_liq_table.npz")

def build_n2o_u_gas_table(
    ...
    np.savez(gas_out_file, T_grid=T_grid, rho_grid=rho_grid, u_gas_tab=u_gas_tab)
    print("saved to:", gas_out_file)
    return rho_grid, T_grid, u_gas_tab

def build_n2o_u_liq_table(
    ...
    np.savez(liq_out_file, T_grid=T_grid, rho_grid=rho_grid, u_liq_tab=u_liq_tab)
    print("saved to:", liq_out_file)
    return rho_grid, T_grid, u_liq_tab

build_n2o_u_gas_table()
build_n2o_u_liq_table()
...
class N2OPLiqTable:
    def __init__(self, path="src/models/_thermo/P_lookup_tables/n2o_P_liq_table.npz"):
        d = np.load(path)
        self.rho_grid = d["rho_grid"]
        self.T_grid = d["T_grid"]
        self.P_liq_tab = d["P_liq_tab"]
    ...
    def lookup(self, rho, T):
        ...
        if np.isnan(Q11) or np.isnan(Q21) or np.isnan(Q12) or np.isnan(Q22):
            print(f"Caution: P_liq lookup returned nan at (rho={rho:.0f} kg/m^3), (T={T:.4f} K), using lib directly (approx .012s per call)")
            return SpanWagnerEOS_SingleState(rho, T).u

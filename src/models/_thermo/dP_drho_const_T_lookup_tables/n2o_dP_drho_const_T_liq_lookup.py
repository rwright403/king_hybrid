...
class N2ODPDRHOTLiqTable:
    def __init__(self, path="src/models/_thermo/dP_drho_const_T_lookup_tables/n2o_dP_drho_const_T_liq_table.npz"):
        d = np.load(path)
        self.rho_grid = d["rho_grid"]
        self.T_grid = d["T_grid"]
        self.dP_drho_const_T_liq_tab = d["dP_drho_const_T_liq_tab"]

    def lookup(self, rho, T):
        rho_grid = self.rho_grid
        T_grid = self.T_grid
        F = self.dP_drho_const_T_liq_tab
        ...
        if np.isnan(Q11) or np.isnan(Q21) or np.isnan(Q12) or np.isnan(Q22):
            print(f"Caution: dP_drho_const_T_liq lookup returned nan at (rho={rho:.0f} kg/m^3), (T={T:.4f} K), using lib directly (approx .012s per call)")
